...
```js {6}
function delayedLoading(fn, n) {
	"use strict";const ctr=new AbortController();const signal=ctr.signal,once=true;let c=false,tId;
	function loadFallback(e){tId = setTimeout(()=>connectScript(e),5000)}
