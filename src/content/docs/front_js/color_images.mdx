---
title: Получение доминирующего цвета из изображения
---

`colorImages(src, opts)` -
Передаем ссылку на изображения и опции(?) (ignore?: string[]; scale?: number; skipTransparentPixels?: boolean;), получаем массив всех цветов от большему к меньшему
...
function getImageData(src: string, scale: number) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const errorHandler = () =>
      reject(new Error("Произошла ошибка при попытке загрузить изображение"));
...
export default async function colorImages(src: string, opts?: Opts) {
  ...
  try {
    ...
    if ((!!scale && scale > 1) || (!!scale && scale <= 0))
      console.warn(`Вы установили масштаб на ${scale}, которое не находится в диапазоне от 0 до 1. Это либо бессмысленно (> 1), либо бесполезно (≤ 0).`);
